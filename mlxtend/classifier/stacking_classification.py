        ...
        if self.fit_base_estimators:
            ...
            for a in range(len(self.clfs_)):
                if sample_weight is None:
                    ...
                    if self.preprocessing:
                        try:
                            self.clfs_[a][0] = self.clfs_[a][0].fit(sm.Preprocessing(X, self.clfs_[a][1]), y,sample_weight)
                        except:
                            self.clfs_.remove(self.clfs_[a])
                            continue
        ...
        if self.use_probas:
            ...
            for clf in self.clfs_:
                if self.preprocessing:
                    try:
                        all_results.append(
                            executor.submit(clf[0].predict_proba,
            ...
            for clf in self.clfs_:
                if self.preprocessing:
                    All_res.append(clf[0].predict(sm.Preprocessing(X, clf[1])))
